...
class AffineTransformed(TransformedDistribution):
    def __init__(
        ...
        super().__init__( #TODO check the base_dist and mean and variance, they are indeed learning how to transform the base mean and variance!
            base_dist,
...
class DistributionOutput:
    ...
    def distribution(
        ...
        if loc is not None or scale is not None:
            distr = AffineTransformed(distr, loc=loc, scale=scale) #adds the transformation for (x)-> loc + scale * x
        return distr
