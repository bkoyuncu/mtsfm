...
class MoiraiPretrain(L.LightningModule):
    seq_fields: tuple[str, ...] = (
        ...
        "prediction_mask",
        "patch_size",
        "prediction_mask_jump" #no need bc we are only using this in loss computation
    )
    ...
    def __init__(
        ...
        sample_per_subset: bool = False,
        subsets_id: str = None,
        rand_cov_indx: bool = True
    ):
    ...
    def forward(
        ...
        # prediction_mask_jump: Bool[torch.Tensor, "*batch seq_len max_patch"],
    ) -> Distribution:
        """
    ...
    def training_step(
        ...
        '''
        in the batch
        target torch.Size([32, 512, 32]) torch.float32
        observed_mask torch.Size([32, 512, 32]) torch.bool
        time_id torch.Size([32, 512]) torch.int64
        variate_id torch.Size([32, 512]) torch.int64
        prediction_mask torch.Size([32, 512]) torch.bool
        patch_size torch.Size([32, 512]) torch.int64
        sample_id torch.Size([32, 512]) torch.int64

        self.seq_fields: ('target', 'observed_mask', 'time_id', 'variate_id', 'prediction_mask', 'patch_size')
        '''#BREAKPOINT training step!

        distr = self( #gets distribution
            **{field: batch[field] for field in list(self.seq_fields) + ["sample_id"] if field != "prediction_mask_jump"}
        )
        loss = self.hparams.loss_func( #loss function, packednllloss, check PackedLoss reduce to see how loss is being scalar, 
            pred=distr,
            **{
                field: batch[field]
                for field in [
                    ...
                    "sample_id",
                    "variate_id",
                    "prediction_mask_jump"
                ]
            },
        ) #returns scalar total over all batches.
        batch_size = (
    ...
    def train_transform_map(self) -> dict[str, Callable[..., Transformation]]:
        ...
        """
        #HERE we add the transormations
        def default_train_transform():
            return (
                SampleDimension(
                    ...
                    per_subset=self.hparams.sample_per_subset,
                    subsets_id= self.hparams.subsets_id
                )
                + GetPatchSize(
                ...
                + AddObservedMask( #TODO:SOLVED double check with missing data, 'observed_mask'['target'] has zeros for nans 
                    fields=("target",),
                    optional_fields=("past_feat_dynamic_real",),
                    observed_mask_field="observed_mask",
                    collection_type=dict,
                )
                + ImputeTimeSeries( #TODO:SOLVED double check with missing data, nans are filled with zero, and we know the indices with observed_mask
                    fields=("target",),
                    optional_fields=("past_feat_dynamic_real",),
                    imputation_method= LastValueImputation(value=0.0)#DummyValueImputation(value=0.0),
                )
                + Patchify(
                    max_patch_size= max(self.module.patch_sizes), #OVERWRITE
                ...
                + AddVariateIndex(
                    ...
                    expected_ndim=3,
                    max_dim=self.hparams.max_dim,
                    randomize=self.hparams.rand_cov_indx,
                    collection_type=dict
                ...
                + MaskedPredictionJump(
                    target_field="target",
                    prediction_mask_field="prediction_mask",
                    predictiom_mask_field_jump="prediction_mask_jump"
                )
                + ExtendMask(
                    fields=tuple(),
                    optional_fields=("past_feat_dynamic_real",),
                ...
                + ExtendMask(
                    fields=tuple(),
                    optional_fields=("past_feat_dynamic_real",),
                    mask_field="prediction_mask_jump",
                    expected_ndim=3,
                )
                + FlatPackCollection(
                    field="variate_id",
                ...
                # + FlatPackFields(
                #     output_field="prediction_mask_jump",
                #     fields=("target",),
                #     optional_fields=(),
                #     feat=False,
                # )
