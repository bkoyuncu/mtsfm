...
class MoiraiFinetune(L.LightningModule):
        ...
        def default_train_transform():
            return (
                SampleDimension(
                    ...
                    use_features= self.hparams.use_features
                )
                + GetPatchSize(
                ...
                + ImputeTimeSeries(
                    fields=("target",),
                    optional_fields=("past_feat_dynamic_real",),
                    imputation_method=LastValueImputation(value=0.0),
                )
                + Patchify(
    ...
    def val_transform_map( #TODO
        ...
        def default_val_transform(
            ...
            context_length: int,
            patch_size: int, #we can add here a scenerios
        ):
            return (
                SampleDimension(
                    max_dim=self.hparams.max_dim,
                    ...
                    optional_fields=("past_feat_dynamic_real",),
                    per_subset=self.hparams.sample_per_subset,
                    subsets_id= self.hparams.subsets_id_val,
                    split = 'val',
                    use_features= self.hparams.use_features
                )
                +
                GetPatchSize(
                    min_time_patches=self.hparams.min_patches, #2 #Can also be good
                    target_field="target",
                    patch_sizes=self.module.patch_sizes,
