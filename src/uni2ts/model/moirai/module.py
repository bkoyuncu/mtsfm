...
class MoiraiModule(
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.d_model = d_model #DEBUG stop here to check pretrained model being load
    ...
    def forward(
        ...
        """ #BREAKPOINT moiria module every computation of forward is here. 
        ...
        scaled_target = (target - loc) / scale #HERE zero mean unit variance computed over the observed ones #if nan target=0, loc for that window=0, scale for that window =1
        reprs = self.in_proj(scaled_target, patch_size) #scaled_target (bs,max_length,patch_size) patch_size (bs, max_length) -> reprs (1,512,384) (512 was max_len)
        #TODO:SOLVED comeback here when we have nans in the input #seems like only for the prediction_mask, this can be thought independent of nansit changes even fully nan channels only the missing windows as mask encoding
        masked_reprs = mask_fill(reprs, prediction_mask, self.mask_encoding.weight) #HERE after we get the projections for every window, we for the missing windows we replace mask_encoding (learnable) for the missing parts.
        reprs = self.encoder(
            masked_reprs, #input bs,512,emd_dim where missing parts are changed with placeholders.
            packed_attention_mask(sample_id), #bs,512,512 
            time_id=time_id,
            var_id=variate_id,
        )#output bs,512,emb_dim
        distr_param = self.param_proj(reprs, patch_size) #returns a dict with 'weights_logits' bs,512,32,4 (here 4 is number of output distributions) 'components' has length of 4 each has didstribution related outputs? #TODO
        distr = self.distr_output.distribution(distr_param, loc=loc, scale=scale) #so they learn to perform affine transform. of emperical mean and var
        return distr #returns an affined transformation of outputs
