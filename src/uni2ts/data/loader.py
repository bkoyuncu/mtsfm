...
class PackCollate(Collate):
    ...
    def __call__(self, batch: list[Sample]) -> BatchedSample:
        ...
        sample_id = self.get_sample_id(packed_batch, bin_spaces) #zeros length of max_length, only ch*#patch are 1s , in packed batch
...
class _BatchedSampleIterator:
    ...
    def _next_batch(self) -> Optional[BatchedSample]:
        ...
        """
        if len(self.queue) < self.batch_size: #HERE #BREAKPOINT QUEUE
            ...
            # except AssertionError:
                ...
                # return None #we try to get next data
