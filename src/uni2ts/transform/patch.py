...
class PatchSizeConstraints(abc.ABC):
    ...
    def __call__(self, freq: str) -> range:
        ...
        start, stop = self._get_boundaries(offset.n, norm_freq_str(offset.name)) #HERE GETS THE OFFSET
...
class GetPatchSize(Transformation):
    #HERE we select the patch sizes, we discussed it as 32, and it adds 'patch_size' key to data entry
    min_time_patches: int
    target_field: str = "target"
    patch_sizes: tuple[int, ...] | list[int] | range = (8, 16, 32, 64, 128)
    patch_size_constraints: PatchSizeConstraints = DefaultPatchSizeConstraints() #DEBUG patch sizes are here
    offset: bool = True
...
class Patchify(MapFuncMixin, Transformation):
    ...
    #HERE we have our patches in 'targets', shape (ch, #patches, #patch_size (length))
    def __call__(self, data_entry: dict[str, Any]) -> dict[str, Any]:
