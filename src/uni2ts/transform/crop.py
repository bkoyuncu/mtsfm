...
class PatchCrop(MapFuncMixin, Transformation):
    ...
    """
    #HERE we crop the target to start end from using patch sizes, including offset
    #it changes 'target' size to total cropped length, not windowed yet.
    #CRITICAL max_patches range is important here! 
    ...
    def _get_boundaries(self, data_entry: dict[str, Any]) -> tuple[int, int]:
        ...
        ) #nvar is total #channels given #if you give 4 features per channel and you select 2 features it is 4*2=8

        #offset from the start, if we have left over with patchsize
        offset = (
            np.random.randint(
                time % patch_size + 1
        ...
        max_patches = min(self.max_patches // nvar, total_patches) #DEBUG
...
class EvalCrop(MapFuncMixin, Transformation):
    ...
    def _get_boundaries(self, data_entry: dict[str, Any]) -> tuple[int, int]:
        field: list[UnivarTimeSeries] = data_entry[self.fields[0]]
        time = field[0].shape[0] #total len
        window = data_entry["window"]
        fcst_start = time - (self.prediction_length + window * self.distance)
